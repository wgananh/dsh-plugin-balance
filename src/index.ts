...
export class BalanceViewModel {
  ...
  constructor(ctx: Context, options: { interval?: number; apiKeyProvider: () => string }) {
    this.ctx = ctx;
  ...
  get refreshInterval(): number { return this.interval; }
...
/** 将余额数据格式化为人类可读文本 */
export function formatBalanceText(data: BalanceResponse): string {
...
function snapshotText(viewModel: BalanceViewModel): string {
...
/** 查询类工具的统一返回结构 */
function toSnapshotResult(viewModel: BalanceViewModel) {
 ...
 */
function apply(ctx: Context, config: BalanceConfig = {}): void {
...
// dsh-commands 服务在 @deepseek-ai/cordis 上的上下文扩展（本包不依赖 dsh-commands 包，仅声明用到的面）
  ...
  interface Context {
    commands: {
      register(definition: {
        ...
        description: string;
        input?: { hint: string };
        handler: (invocation: { commandId: string; rawInput: string; signal: AbortSignal }) =>
